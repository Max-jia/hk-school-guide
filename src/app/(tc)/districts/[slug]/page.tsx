...
import { DISTRICTS, districtMetadata } from "@/lib/districts";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return districtMetadata(slug, "tc");
}

function SchoolRow({ s, isKg, locale = "tc" }: { s: any; isKg: boolean; locale?: Locale }) {
  ...
  if (!d) {
    return (
      <main className="w-full">
        ...
        <p className="p-8 text-[var(--p-secondary)]">{L("找不到该地区。", locale)}</p>
      ...
      <div className="mx-auto max-w-[1080px] px-4 pb-24">
        <div className="py-8">
          <p className="font-mono text-sm uppercase text-[var(--p-secondary)]">
            <a href={localeHref("/districts", locale)} className="text-[var(--p-secondary)] no-underline hover:underline">分区盘点</a> · {d.en}
          </p>
        ...
        <div className="mt-2 overflow-x-auto">
          <table className="w-full min-w-[760px] border-collapse">
            ...
            <tbody>
              {ps.map((s, i) => (
                <SchoolRow key={i} s={s} isKg={false} locale={locale} />
              ))}
            </tbody>
          </table>
        </div>
        ...
        <div className="mt-2 overflow-x-auto">
          <table className="w-full min-w-[760px] border-collapse">
            ...
            <tbody>
              {kg.map((k, i) => (
                <SchoolRow key={i} s={k} isKg={true} locale={locale} />
              ))}
            </tbody>
          </table>
        </div>
