...
- **滑档线热门校修正**：学额多 ≠ 容易进。喇沙（90 学额）这类传统热门校此前被按「充裕/安全」处理，15 分填喇沙会误输出「第 1 位起相对安全」。已加传统热门校名单（29 所，基于常见择校讨论整理，非官方热度数据），热门校一律按「抽签区」处理，不进入相对安全位判定。
- 只填 1 所（尤其热门校）提示改为「学额 90 但属传统热门校——谈不上滑档：这所不中就没有第二个落点」。
- 「什么是滑档线」解释更新为「大风吹」类比 + 明确「学额多≠容易进」。
- 底牌卡同步热门修正：20 分 + 喇沙 → 「基本靠抽签」，不再显示「学额仅 90」。
- 实测：41 网喇沙/玛利诺/圣家/华德 → 滑档线落第 4 位（华德），符合直觉；单文件版同步。
