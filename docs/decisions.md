## 2026-09-08 模拟器升级 + 兑换码 + 单文件版
- **模拟器新增 3 项检查**：1-1-1 诚意矩阵（甲一/乙一是否同一学校）、乙一撞热门预警（冲刺档+计分≤20）、乙二宜守不宜攻（乙二冲刺 → 滑档风险）——已上线
- **兑换码系统**：`/api/redeem` + `/redeem` 页面 + `scripts/gen-redeem-codes.mjs`（Upstash KV 存储，SET NX 防重复核销，码格式 SIM-XXXX-XXXX，有效期 1 年）
- **单文件版模拟器**：`output/standalone/小一派位模拟器.html`（自包含 39KB、433 校内联、无外部依赖、红绿灯检查、localStorage 草稿、可打印 PDF）——小红书可直接交付文件
- 测试：消耗 2 个测试码（4U5Z-MFM5、WYRQ-TKHJ），剩余库存 1 个（USLP-ND4X）
- 定价建议：单文件版 ¥49-59；兑换码 ¥59-69；与站内 HK$68 报告并存，各走各渠道
