#!/usr/bin/env python3
"""报告 HTML 骨架生成器。输入学校代码 + 基础数据 → 输出完整 HTML 骨架。
   Claude 只需填充 8 章内容即可。"""
import json, sys, os

def load_school(code):
    """从数据库加载学校数据"""
    os.chdir(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    with open('data/schools_primary.json') as f:
        schools = json.load(f)
    with open('data/primary_tiers_v9.json') as f:
        tiers = json.load(f)
    score_map = {s['name']: s for s in tiers['scores']}

    for s in schools:
        if s.get('name_zh') == code:
            ti = score_map.get(s['name_zh'], {})
            return {**s, **ti}
    return None

def gen_shell(school, output_path):
    """生成包含所有固定组件的 HTML 骨架"""
    name = school['name_zh']
    code = school.get('name_en','').lower().replace(' ','-').replace("'",'')[:20]
    tier = school.get('tier','B')
    tier_class = {'S':'s','A+':'aplus','A':'a','B':'b'}.get(tier,'b')
    tier_label = {'S':'🏆 S 级','A+':'🥇 A+ 级','A':'🌟 A 级','B':'✅ B 级'}.get(tier,'B 级')
    ...
    if net: sub += f'({net}校网)'
    if relation and best_sec: sub += f' · {relation}{best_sec}(Band 1)'
...
<footer class="footer">
  <div class="container" style="text-align:center">
    ...
    print(f'   学校: {name} | 等级: {tier} | 总分: {total}')
    print(f'   联系: {relation} {best_sec}')
    return html

if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('用法: python3 scripts/gen_report_template.py "学校全名" <code>')
        print('例如: python3 scripts/gen_report_template.py "聖羅撒學校" srl')
        sys.exit(1)
    school = load_school(sys.argv[1])
    if not school:
        print(f'❌ 未找到学校: {sys.argv[1]}')
        sys.exit(1)
    code = sys.argv[2]
    gen_shell(school, f'report-{code}.html')
